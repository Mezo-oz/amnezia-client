...
bool WireguardUtilsLinux::deleteInterface() {
    ...
    if (m_tunnel.state() == QProcess::NotRunning) {
        ...
        // interface was removed out from under it. Returning here without
        // dropping the firewall leaves the kill switch rules loaded with no
        // tunnel to protect, so the user is left with no network and no way to
        // clear it from the UI. There is no traffic left to leak at this point,
        // so it is safe to tear the firewall down now.
